  ...
  void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
    ...
    if(rssi > -45)
    {
        ...
        if (advertisedDevice->haveServiceUUID()) {
            ...
            Serial.printf("  Service UUID: none\n");
        }
        ...
        if (advertisedDevice->haveManufacturerData()) {
            ...
            Serial.printf("  Manufacturer Data: none\n");
        }
        Serial.printf("===========================\n\n");
    }
    ...
    // 方式1: 通过MAC地址匹配（如果提供了MAC地址）
    if (targetDeviceAddress != nullptr &&
        advertisedDevice->getAddress().equals(*targetDeviceAddress)) {
      isTargetDevice = true;
      matchReason = "MAC address match";
    }
    // 方式2: 通过设备名称"Xbox Wireless Controller"自动匹配
    else if (targetDeviceAddress == nullptr &&
             strcmp(advertisedDevice->getName().c_str(), "Xbox Wireless Controller") == 0) {
      isTargetDevice = true;
      matchReason = "Device name 'Xbox Wireless Controller'";
    }
    // 方式3: 通过外观值964（游戏手柄）和HID服务匹配
    else if (targetDeviceAddress == nullptr &&
             advertisedDevice->getAppearance() == controllerAppearance &&
             advertisedDevice->haveServiceUUID() &&
             advertisedDevice->getServiceUUID().equals(uuidServiceHid)) {
      isTargetDevice = true;
      matchReason = "Appearance 964 (Gamepad) + HID Service";
    }
    // 方式4: 通过外观和制造商数据匹配（原有逻辑）
    else if (targetDeviceAddress == nullptr &&
             advertisedDevice->getAppearance() == controllerAppearance &&
             ...
             advertisedDevice->getServiceUUID().equals(uuidServiceHid)) {
      isTargetDevice = true;
      matchReason = "Appearance + Manufacturer data + HID Service";
    }
    
    if (isTargetDevice) {
      Serial.printf("\n*** TARGET CONTROLLER FOUND! ***\n");
